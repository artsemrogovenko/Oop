...
import static com.mygdx.game.Persons.Positions.distance;
...
public class Move {
    ...
    private void selectDirection(BaseHero me, BaseHero target, ArrayList<BaseHero> team) {
        ...
        speedmove(me.getPos(),target.getPos(), me.getSpeed());
        if (dx > dy) {
            if (x < targX) {
                direction("right");
            }
    ...
    private void speedmove(int[] start, int[] end,int heroSpeed) {
        time=distance(start,end)  / heroSpeed;
        System.out.println("время для новой позиции" + time);
    }
}
